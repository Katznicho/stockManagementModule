...
class StockManagementController extends Controller
{
    public function storeBulkOrder(Request $request)
    {
        try {
            ...
            // Prepare the response payload with predictions
            $payload = [
                'branch_id' => $validatedData['branch_id'],
                'items' => $validatedData['items'],
                'order_by_days' => $validatedData['order_by_days'],
                'order_by_budget' => $validatedData['order_by_budget'],
                'bulk_order_date' => $validatedData['bulk_order_date'],
                'timestamp' => now()->toIso8601String(),
                'external_id' => auth()->user()->entity_id ?? null, // Include external ID from authenticated user
                'predictions' => [], // Array of predictive data for each item
            ];
